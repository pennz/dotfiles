...
# Cleaned up by pennz (2020) follow the suggestion by shellcheck.
...
if [ "$#" -ge 1 ]; then
    ...
    if tty -s; then
        SQLFILE=$(find ~/.mozilla/firefox/*/cookies.sqlite | head -1)
    else
...
if [ "$W3M_FORMAT" -eq 1 ]; then
    # website url, name, value, expiry, host, path, 
, secure
